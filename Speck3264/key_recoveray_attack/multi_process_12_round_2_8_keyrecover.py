...
# m6 = np.load(wdir+"data_wrong_key_mean_6r_pairs"+str(pairs)+".npy");
# s6 = np.load(wdir+"data_wrong_key_std_6r_pairs"+str(pairs)+".npy"); 
...
def test_correct_pairs(pt0a,pt1a,key, nr=3,diff=(0x211,0xa04),target_diff=(0x0040, 0x0)):
    
    
    pt0b = pt0a ^ diff[0]
    pt1b = pt1a ^ diff[1]
    pt0a_1, pt1a_1 = sp.dec_one_round((pt0a, pt1a), 0)
    pt0b_1, pt1b_1 = sp.dec_one_round((pt0b, pt1b), 0)

    ct0a, ct1a = sp.encrypt((pt0a_1, pt1a_1), key[:nr])
    ct0b, ct1b = sp.encrypt((pt0b_1, pt1b_1), key[:nr])
    diff0 = ct0a ^ ct0b
    diff1 = ct1a ^ ct1b
...
def gen_challenge(pt0,pt1,key, nr, diff=(0x0211,0x0a04), neutral_bits=[20, 21, 22, 14, 15, 23]):
    #           明文对的数量 轮数          差分             中立bit                           密钥扩展算法

    # print("gen_challenge......")
    # 随机生成明文，
    # pt0 shape 应该是pairs*num_structure
    # print("pt0 shape",pt0.shape)
    # 利用中立bit生成结构密文对
    pt0a, pt1a, pt0b, pt1b = make_structure(
        pt0, pt1, diff=diff, neutral_bits=neutral_bits)
    # print("pt0a shape",pt0a.shape)
    # 这是就是无花销扩展一轮的操作吗？
...
def test_bayes(device,cts,pairs,it=1, cutoff1=10, cutoff2=10, net=net8, net_help=net7, m_main=m8, m_help=m7, s_main=s8, s_help=s7, verify_breadth=None):
...
def test(device,n=5, pairs=8,nr=12, num_structures=500, it=3000, cutoff1=-5, cutoff2=-150, neutral_bits=[[9,16],[2,11,25],[14],[15],[6,29],[23],[30],[7],[0],[11,27]], net=net8, net_help=net7, m_main=m8, s_main=s8,  m_help=m7, s_help=s7, verify_breadth=None):
    ...
    for i in range(n):
        ...
        pt0,pt1,_,_ = make_structure(pt0,pt1,neutral_bits=[[20],[21],[22]])
        pt0 = pt0.transpose().flatten()
        pt1 = pt1.transpose().flatten()

        ct = gen_challenge(
            pt0,pt1,key,nr, neutral_bits=neutral_bits)
...
if __name__ == "__main__":
    ...
    neutral_bits = [[20],[21],[22],[14],[15],[23],[30],[7],[0]]
