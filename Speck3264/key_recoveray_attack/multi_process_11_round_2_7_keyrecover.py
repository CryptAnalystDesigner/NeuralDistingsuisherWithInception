...
def test_correct_pairs(pt0l,pt0r,key, nr=3,diff=(0x0211,0x0a04),target_diff=(0x0040, 0x0)):
...
def gen_challenge(pt0,pt1,key, nr, diff=(0x0211,0x0a04), neutral_bits=[20, 21, 22, 14, 15, 23], keyschedule='real'):
...
def verifier_search(pairs,cts, best_guess, use_n=2**num_neural_bit, net=net7):
    # 进来的数据就是一个密文结构内的数据
...
def test_bayes(device,cts,pairs,it=1, cutoff1=10, cutoff2=10, net=net7, net_help=net6, m_main=m7, m_help=m6, s_main=s7, s_help=s6, verify_breadth=None):
    ...
    for j in range(it):
        ...
        if (best_val > cutoff2):
            ...
            while improvement:
                ...
                print("the device " +str(device) + " val = "+str(val))
...
def test(device,n=5, pairs=8,nr=11, num_structures=300, it=500, cutoff1=140, cutoff2=180, neutral_bits = [[20],[22],[21],[9,16],[2,11,25],[14]], keyschedule='real', net=net7, net_help=net6, m_main=m7, s_main=s7,  m_help=m6, s_help=s6, verify_breadth=None):
    ...
    for i in range(n):
        ...
        ct = gen_challenge(
            pt0,pt1,key,nr, neutral_bits=neutral_bits, keyschedule=keyschedule)
