...
m10 = np.load(mswdir+"simon_model_10r_num_epochs40_acc_0.676828_data_wrong_key_mean_10r_pairs8.npy");
s10 = np.load(mswdir+"simon_model_10r_num_epochs40_acc_0.676828_data_wrong_key_std_10r_pairs8.npy"); 
...
tmp_br = np.arange(2**13, dtype=np.uint16)
...
def bayesian_key_recovery(pairs,cts, net=net11, m=m11, s=s11, num_cand=num_cand, num_iter=5, seed=None):
    ...
    keys = np.random.choice(2**(WORD_SIZE-3), num_cand, replace=False)
    scores = 0
    best = 0
    if (not seed is None):
        keys = np.copy(seed)
    ...
    # scores = np.zeros(2**(WORD_SIZE-3))
    # used = np.zeros(2**(WORD_SIZE-3))
    # 32*5=160个密钥
    ...
    for i in range(num_iter):
        ...
        # 重置密钥
        keys = tmp[0:num_cand]
        r = np.random.randint(0, 8, num_cand, dtype=np.uint16)
        r = r << 13
        keys = keys ^ r
    ...
    for j in range(it):
        ...
        if (best_val > cutoff2):
            ...
            while improvement:
                # best_pod为不同的密文结构
                # 到了最后阶段，验证猜测的密钥是否正确
                print("cut off the device " +str(device) + " best_pod = "+str(best_pod))
...
def test(device,n=20, pairs=8,nr=17, num_structures=2**11, it=2**8, cutoff1=300, cutoff2=150, net=net11, net_help=net10, m_main=m11, s_main=s11,  m_help=m10, s_help=s10, verify_breadth=None):
    ...
    net = load_model(model_wdir+"simon_model_11r_num_epochs40_acc_0.558271.h5")
    net_help = load_model(model_wdir+"simon_model_10r_num_epochs40_acc_0.676828.h5")
