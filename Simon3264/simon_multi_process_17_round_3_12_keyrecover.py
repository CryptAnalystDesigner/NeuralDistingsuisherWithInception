...
m12 = np.load(mswdir+"simon_data_wrong_key_mean_"+str(rounds)+"r_pairs"+str(pairs)+".npy");
s12 = np.load(mswdir+"simon_data_wrong_key_std_"+str(rounds)+"r_pairs"+str(pairs)+".npy"); 
m11 = np.load(mswdir+"simon_data_wrong_key_mean_"+str(rounds-1)+"r_pairs"+str(pairs)+".npy");
s11 = np.load(mswdir+"simon_data_wrong_key_std_"+str(rounds-1)+"r_pairs"+str(pairs)+".npy"); 
...
def test(device,n=1, pairs=8,nr=17, num_structures=2**9, it=300, cutoff1=60, cutoff2=350, neutral_bits=[[2],[3],[4],[6],[8],[9],[10],[18],[22],[0,24],[12,26]], net=net12, net_help=net11, m_main=m12, s_main=s12,  m_help=m11, s_help=s11, verify_breadth=None):
