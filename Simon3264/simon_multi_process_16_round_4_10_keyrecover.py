...
m10 = np.load(mswdir+"simon_data_wrong_key_mean_"+str(rounds)+"r_pairs"+str(pairs)+".npy");
s10 = np.load(mswdir+"simon_data_wrong_key_std_"+str(rounds)+"r_pairs"+str(pairs)+".npy"); 
m9 = np.load(mswdir+"simon_data_wrong_key_mean_"+str(rounds-1)+"r_pairs"+str(pairs)+".npy");
s9 = np.load(mswdir+"simon_data_wrong_key_std_"+str(rounds-1)+"r_pairs"+str(pairs)+".npy"); 
...
def verifier_search(pairs,cts, best_guess, use_n=2**num_neural_bit, net=net10):
    # 进来的数据就是一个密文结构内的数据
...
def bayesian_rank_kr(cand, emp_mean, m=m10, s=s10):
...
def bayesian_key_recovery(pairs,cts, net=net10, m=m10, s=s10, num_cand=32, num_iter=5, seed=None):
...
def test_bayes(device,cts,pairs,it=1, cutoff1=10, cutoff2=10, net=net10, net_help=net9, m_main=m10, m_help=m9, s_main=s10, s_help=s9, verify_breadth=None):
...
def test(device,n=5, pairs=8,nr=16, num_structures=2**10, it=2**11, cutoff1=25, cutoff2=60, net=net10, net_help=net9, m_main=m10, s_main=s10,  m_help=m9, s_help=s9, verify_breadth=None):
