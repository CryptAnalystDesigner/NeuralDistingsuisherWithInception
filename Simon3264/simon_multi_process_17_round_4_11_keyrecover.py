...
m11 = np.load(mswdir+"simon_data_wrong_key_mean_"+str(rounds)+"r_pairs"+str(pairs)+".npy");
s11 = np.load(mswdir+"simon_data_wrong_key_std_"+str(rounds)+"r_pairs"+str(pairs)+".npy"); 
m10 = np.load(mswdir+"simon_data_wrong_key_mean_"+str(rounds-1)+"r_pairs"+str(pairs)+".npy");
s10 = np.load(mswdir+"simon_data_wrong_key_std_"+str(rounds-1)+"r_pairs"+str(pairs)+".npy"); 
...
def make_structure(pt0, pt1, diff=(0x1000,0x4440), neutral_bits=[[20],[21],[22],[9,16],[2,11,25],[14],[15],[6,29]]):
...
# def test_correct_pairs(pt0l,pt0r,key, nr=5,diff=(0x1000,0x4440),target_diff=(0x0000, 0x0040)):
...
def test_correct_pairs(ct0l,ct0r,ct1l,ct1r,key, nr=5,target_diff=(0x0000, 0x0040)):
    
    pt0l, pt0r = si.decrypt((ct0l,ct0r), key[nr:])
    pt1l, pt1r = si.decrypt((ct1l,ct1r), key[nr:])
    diff0 = pt0l ^ pt1l
    diff1 = pt0r ^ pt1r
...
def gen_challenge(pt0a,pt1a,pt0b, pt1b, key):
...
def verifier_search(pairs,cts, best_guess, use_n=2**num_neural_bit, net=net11):
    # 进来的数据就是一个密文结构内的数据
...
def bayesian_key_recovery(pairs,cts, net=net11, m=m11, s=s11, num_cand=num_cand, num_iter=5, seed=None):
...
def test_bayes(device,cts,pairs,it=1, cutoff1=10, cutoff2=10, net=net11, net_help=net10, m_main=m11, m_help=m10, s_main=s11, s_help=s10, verify_breadth=None):
    ...
    # 密文结构
    best_pod = 0
    keys = np.random.choice(2**WORD_SIZE, num_cand, replace=False)
    eps = 0.001
    # 第i个密文结构最高的区分器得分
    ...
    for j in range(it):
        ...
        keys, scores, v = bayesian_key_recovery(
            pairs,[cts[0][i::n], cts[1][i::n], cts[2][i::n], cts[3][i::n]], num_cand=num_cand, num_iter=5, net=net, m=m_main, s=s_main)
        vtmp = np.max(v)
        # if vtmp > 0:
        #     print("vtmp = ",vtmp)
        # print("vtmp = ",vtmp)
        ...
        if (vtmp > cutoff1):
            ...
            for i2 in l2:
                ...
                keys2, scores2, v2 = bayesian_key_recovery(
                    pairs,[c0a, c1a, c0b, c1b], num_cand=num_cand, num_iter=5, m=m_help, s=s_help, net=net_help)
                vtmp2 = np.max(v2)
...
def select_neutral_bit(pt0i,key,nb):

    set_bit_x1 = GET1(key[0],1) ^ GET1(pt0i,1)
    set_bit_x15 = GET1(key[0],15) ^ GET1(pt0i,15)
    set_bit_x13 = GET1(key[0],13) ^ GET1(pt0i,13)
    set_bit_x11 = GET1(key[0],11) ^ GET1(pt0i,11)
    set_bit_x9 = GET1(key[0],9) ^ GET1(pt0i,9)
    set_bit_x7 = GET1(key[0],7) ^ GET1(pt0i,7)

    if set_bit_x1 ==0 and set_bit_x15 ==0:
        nb.append([24,10])
    elif set_bit_x1 ==1 and set_bit_x15 ==0:
        nb.append([24,10,9])
    elif set_bit_x1 ==0 and set_bit_x15 ==1:
        nb.append([24,10,0])
    elif set_bit_x1 ==1 and set_bit_x15 ==1:
        nb.append([24,10,9,0])

    if set_bit_x15 ==0 and set_bit_x13 ==0:
        nb.append([22,8])
    elif set_bit_x15 ==1 and set_bit_x13 ==0:
        nb.append([22,8,7])
    elif set_bit_x15 ==0 and set_bit_x13 ==1:
        nb.append([22,8,14])
    elif set_bit_x15 ==1 and set_bit_x13 ==1:
        nb.append([22,8,7,14])

    if set_bit_x13 ==0 and set_bit_x11 ==0:
        nb.append([20])
    elif set_bit_x13 ==1 and set_bit_x11 ==0:
        nb.append([20,5])
    elif set_bit_x13 ==0 and set_bit_x11 ==1:
        nb.append([20,12])
    elif set_bit_x13 ==1 and set_bit_x11 ==1:
        nb.append([20,12,5])

    if set_bit_x11 ==0 and set_bit_x9 ==0:
        nb.append([18,4])
    elif set_bit_x11 ==1 and set_bit_x9 ==0:
        nb.append([18,4,3])
    elif set_bit_x11 ==0 and set_bit_x9 ==1:
        nb.append([18,4,10])
    elif set_bit_x11 ==1 and set_bit_x9 ==1:
        nb.append([18,4,3,10])

    if set_bit_x9 ==0 and set_bit_x7 ==0:
        nb.append([16,8])
    elif set_bit_x9 ==1 and set_bit_x7 ==0:
        nb.append([16,8,1])
    elif set_bit_x9 ==0 and set_bit_x7 ==1:
        nb.append([16])
    elif set_bit_x9 ==1 and set_bit_x7 ==1:
        nb.append([16,1])
    
    return nb

def test(device,n=5, pairs=8,nr=17, num_structures=2**10, it=2**11, cutoff1=40, cutoff2=125, net=net11, net_help=net10, m_main=m11, s_main=s11,  m_help=m10, s_help=s10, verify_breadth=None):
    #         轮数  结构数                迭代轮数    c1界          c2界             中立
    # bit                        密钥扩展算法      主区分器    辅助区分器
  
    # n是要测试的次数
    gpus = tf.config.list_physical_devices(device_type = 'GPU')
    tf.config.set_visible_devices(devices=gpus[device],device_type='GPU')
    model_wdir = './good_trained_nets/'
    # tf.config.experimental.set_memory_growth(devices=gpus[device],enable = True)
    # net8 = load_model(wdir+"model_8r_depth5_num_epochs20_pairs"+str(pairs)+".h5")

    net = load_model(model_wdir+"simon_model_"+str(nr-6)+"r_depth5_num_epochs20_pairs"+str(pairs)+".h5")
    net_help = load_model(model_wdir+"simon_model_"+str(nr-6-1)+"r_depth5_num_epochs20_pairs"+str(pairs)+".h5")
    ...
    for i in range(n):
        ...
        set_bit_x5 = GET1(key[0],5)
        pt0 = replace_1bit(pt0,set_bit_x5,5)
        pt1 = replace_1bit(pt1,set_bit_x5,5)
        ...
        for j in range(num_structures):
            ...
            neutral_bits=[[12,26],[10,14,28]]
            nb = select_neutral_bit(pt0[j],key,neutral_bits)
  
            pt0i,pt1i,_,_ = make_structure(pt0[j],pt1[j],neutral_bits=[[2],[6],[12,26]])
            # print("nb = ",nb)
            pt0ai, pt1ai, pt0bi, pt1bi = make_structure(pt0i, pt1i, diff=(0x1000,0x4440),     neutral_bits=nb)
            tmpct.append(gen_challenge(pt0ai, pt1ai, pt0bi, pt1bi,key))
            len_nb = len(nb)
        # print("len_nb = ", len_nb)
        tmpct = np.array(tmpct)
        # 6是中立比特的数量
        tmpct = tmpct.transpose((1,2,0,3)).reshape(4,-1,2**(len_nb))
        # print("tmpct shape", tmpct.shape)
        ct = [tmpct[0],tmpct[1],tmpct[2],tmpct[3]]

        td = test_correct_pairs(ct[0][:num_structures,0],ct[1][:num_structures,0],ct[2][:num_structures,0],ct[3][:num_structures,0], key)
...
if __name__ == "__main__":
    ...
    neutral_bits = [[2],[6],[12,26],[10,14,28],[24,10],[22,8],[20],[18,4],[16,8]]
